...
# First party modules
from pylotfish import pylotfish_cli


def test_author() -> None:
    """Test function to assert correct author name."""
    runner = CliRunner()
    result = runner.invoke(pylotfish_cli.author)
...
def test_author_verbose() -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(pylotfish_cli.author, ["--verbose"])
    assert result.exit_code == 0
    assert (
...
def test_author_help() -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(pylotfish_cli.author, ["--help"])
    assert result.exit_code == 0
